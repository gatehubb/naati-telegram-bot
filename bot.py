...
# متن استاندارد منوی اصلی با اضافه شدن قابلیت نمایش زمان تهران و تاریخ شمسی
MAIN_MENU_TEXT = (
    "🤖 <b>دستیار هوشمند پایش آزمون NAATI CCL</b>\n\n"
    "<b>امکانات ربات:</b>\n"
    "• دریافت زنده تاریخ‌های فعال آزمون فارسی\n"
    "• <b>نمایش خودکار تاریخ شمسی و ساعت به وقت ایران (تهران) </b>\n"
...
def _reset_admin_attempts_sync(chat_id):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            "DELETE FROM admin_attempts WHERE chat_id = ?", (chat_id,)
        )
        conn.commit()
...
class StatusTracker:
    ...
    async def update(self, step_text, status="in_progress", error_msg=None):
        ...
        full_text = (
            "⚙️ <b>وضعیت پردازش:</b>\n"
            "<i>(این عملیات ممکن است حدود ۱ دقیقه زمان ببرد، لطفاً منتظر"
            " بمانید...)</i>\n\n"
            + "\n".join(self.steps)
        )
        try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_welcome_text = (
        ...
        "<b>امکانات ربات:</b>\n"
        "• دریافت زنده تاریخ‌های فعال آزمون فارسی\n"
        "• <b>نمایش خودکار تاریخ شمسی و ساعت به وقت ایران (تهران) روی دکمه‌ها و پیام‌ها</b>\n"
        "• پایش یک تاریخ خاص همراه با اعلام تاریخ‌های جدید\n"
        "• پایش همزمان چندین تاریخ (تا ۴ تاریخ)\n"
...
async def handle_text_buttons(
    ...
    if text in [
        ...
        main_kb = await get_main_inline_keyboard(chat_id)
        await update.message.reply_text(
            MAIN_MENU_TEXT, parse_mode="HTML", reply_markup=main_kb
        )
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "btn_list":
        ...
        if idx in selections:
            ...
            if len(selections) >= 4:
                await query.answer(
                    "⚠️ حداکثر می‌توانید ۴ تاریخ را انتخاب کنید!",
                    show_alert=True,
                )
                return
            selections.add(idx)
...
async def render_multi_select_menu(query, context, chat_id, edit=False):
    ...
    keyboard.append([
        InlineKeyboardButton(
            f"📥 ثبت نهایی انتخاب‌ها ({len(selections)}/4)",
            ...
            for chat_id, monitor_info in monitors.items():
                ...
                if mode == "single":
                    ...
                    if current_idx is not None:
                        ...
                        if curr_seats != last_seats:
                            await save_monitor(
                                ...
                                cached_snapshot=",".join(
                                    monitor_info["cached_snapshot"]
                                ),
                                error_notified=0,
                            )
                    ...
                    new_dates_found = [
                        ...
                        for item in data
                        if not any(
                            is_match(item["date"], cd) for cd in cached_dates
                        )
                    ]
                    if new_dates_found:
                        formatted_new = []
                    ...
                    if changes:
                        new_seats_str = " | ".join(updated_seats_list)
                        await save_monitor(
                            chat_id,
                            monitor_info["username"],
                            mode,
                        ...
                        await send_alert(
                            app,
                            chat_id,
                            "🔔 <b>تغییر ظرفیت در تاریخ‌های انتخابی"
                            " شما:</b>\n\n"
                            + "\n".join(changes),
                        )
        except Exception as loop_e:
            logging.error(f"Error in global monitoring loop: {loop_e}")
